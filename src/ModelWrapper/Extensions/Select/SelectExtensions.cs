...
namespace ModelWrapper.Extensions.Select
{
    ...
    public static class SelectExtensions
    {
        ...
        ) where TModel : class
        {
            ...
            List<SelectedProperty> properties = typeof(TModel).GetProperties().Where(x =>
            ...
            ).Select(x=>new SelectedProperty { PropertyName = x.Name, PropertyType = x.PropertyType, PropertyInfo = x }).ToList();

            var doTheThing = typeof(TModel).Name.Equals("Product");
            if (doTheThing)
            {
                var xp = typeof(TModel).GetProperty("Category").PropertyType.GetProperty("Id");
                properties.Add(new SelectedProperty { PropertyName = "category.id", PropertyType = xp.PropertyType, PropertyInfo = xp });
            }

            source.RequestObject.SetValue(Constants.CONST_RESPONSE_PROPERTIES, properties.Select(x => x.PropertyName.ToCamelCase()));
        ...
        {
            return source.ResponseProperties().Any(property => property.PropertyName.ToLower().Equals(name.ToLower()));
        }
